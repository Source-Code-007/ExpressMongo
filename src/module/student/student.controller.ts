...
const createStudentController= async(req:Request, res:Response)=> {
    ...
    try {
        ...
        res.status(200).send({success:true, message:error.message, error:error})
    }
}
...
const getAllStudentsController= async(req:Request, res:Response)=> {
    ...
    try {
        ...
        if(result){
            res.status(200).send({success:true, message:'Students are retrieved successfully', data:result})
        }
...
const getStudentByIdController= async(req:Request, res:Response)=> {
    try {
        ...
        if(true){
            res.status(200).send({success:true, message:'Student is retrieved successfully', data:result})
        }
...
const deleteStudentByIdController= async(req:Request, res:Response)=> {
    try {
        ...
        if(true){
            res.status(200).send({success:true, message:'Student is deleted successfully', data:result})
        }
...
export {createStudentController, getAllStudentsController, getStudentByIdController, deleteStudentByIdController, deleteAllStudentsController}
